...
class Tracker(object):
    ...
    def initialize(self, image, info: dict) -> dict:
        tic = time.time()
        hanning = np.hanning(copy.deepcopy(self.instance_size)//8)
    ...
    def track_neighbor(self, image, neighbor_th):
        self.net.template(self.template)
        ...
        best_idx = np.argmax(pscore)
        bbox = copy.deepcopy(pred_bbox[:, best_idx])
        bbox = bbox * s_x
        cx = bbox[0] + self.center_pos[0] - s_x / 2
        cy = bbox[1] + self.center_pos[1] - s_x / 2
        width = bbox[2]
        ...
        #score[best_idx]*neighbor_th
        
        neighbors_value = cv2.inRange(pscore,pscore[best_idx]*neighbor_th,1.01)
        neighbors_index = cv2.findNonZero(neighbors_value)
        #print(neighbors_index)
        #print(score.shape)
        #print(neighbors_index[0][0][1])
        
        
        neighbors_value_only = [pscore[x[0][1]] for x in neighbors_index]
        neighbors_xywh=[pred_bbox[:, x[0][1]] for x in neighbors_index]
        nbox=[]
        nboxscore=[]
        for neighbor_index,box in enumerate(neighbors_xywh):
            #print('-------')
            #print(neighbors_xywh)
            ...
            ncx = box[0] + self.center_pos[0] - s_x / 2
            ncy = box[1] + self.center_pos[1] - s_x / 2
            nwidth = box[2]
            nheight = box[3]
            #print(neighbors_index[0])
            #print(box[0])
            #print(neighbors_xywh[0])
            #print(self.center_pos[0])

            #print(ncx)
            #print(ncy)
            ncx, ncy, nwidth, nheight = self._bbox_clip(ncx, ncy, nwidth,
                                                    nheight, image.shape[:2])
            nbbox = [ncx - nwidth / 2,
                    ncy - nheight / 2,
                    nwidth,
                    nheight]
            nbox.append(nbbox)
            nboxscore.append(neighbors_value_only[neighbor_index])

        
        return bbox,pscore[best_idx],nbox,nboxscore
