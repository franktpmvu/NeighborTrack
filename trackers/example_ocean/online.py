...
seed = 1234#1234 in 220110
#seed = 1235#1234 in 220110
...
class ONLINE(object):
    ...
    def neighbor_track(self, bgr_im, image, siam_tracker, siam_state,neighbortrack_args) -> dict:
        self.debug_info = {}
        ...
        if 'delay_start' not in neighbortrack_args:
            neighbortrack_args['delay_start']=False
            
        if 'nms_sigma' not in neighbortrack_args:
            neighbortrack_args['nms_sigma'] = 0.01
            
        if 'inv_start_from_history' not in neighbortrack_args:
            neighbortrack_args['inv_start_from_history']=False
            
        if 'history_dict' not in neighbortrack_args:
            neighbortrack_args['history_dict'] = None
        if 'max_consecutive' not in neighbortrack_args:
            neighbortrack_args['max_consecutive'] = -1
        if 'pred_margin_ratio' not in neighbortrack_args:
            neighbortrack_args['pred_margin_ratio'] = 1.0
            print('not input pred_margin_ratio')
        if 'neighbor_search_ratio' not in neighbortrack_args:
            neighbortrack_args['neighbor_search_ratio'] = 1.0
        if 'neighbor_score_only' not in neighbortrack_args:
            neighbortrack_args['neighbor_score_only'] = False
        if 'neighbor_static_th' not in neighbortrack_args:
            neighbortrack_args['neighbor_static_th'] = False
        if 'neighbor_th' not in neighbortrack_args:
            neighbortrack_args['neighbor_th'] = 0.7
        if 'group_inv' not in neighbortrack_args:
            neighbortrack_args['group_inv'] = False
        if 'nms_th' not in neighbortrack_args:
            neighbortrack_args['nms_th'] = 0.25
        ...
        self.siam_state = siam_tracker._neighbor_track(siam_state, bgr_im, online_score=s,warning_inv_th=neighbortrack_args['warning_inv_th'],inv_error_th=neighbortrack_args['inv_error_th'],gtchoose=neighbortrack_args['gtchoose'],just_kalman=neighbortrack_args['just_kalman'],neighbor_2pass_mode=neighbortrack_args['neighbor_2pass_mode'],neighbor_predandinv_mode=neighbortrack_args['neighbor_predandinv_mode'],one_target_pred_first=neighbortrack_args['one_target_pred_first'],kalman_add_mode=neighbortrack_args['kalman_add_mode'],ls_add_mode=neighbortrack_args['ls_add_mode'],inv_frames=neighbortrack_args['inv_frames'],ignore_frames=neighbortrack_args['ignore_frames'],kalman_neighbor_mode=neighbortrack_args['kalman_neighbor_mode'],time_dilate=neighbortrack_args['time_dilate'], gt=neighbortrack_args['gt'],pass_check_mode=neighbortrack_args['pass_check_mode'],dynamic_ig=neighbortrack_args['dynamic_ig'],now_frame_index=neighbortrack_args['now_frame_index'],nms_th=neighbortrack_args['nms_th'],nms_sigma=neighbortrack_args['nms_sigma'],delay_start=neighbortrack_args['delay_start'],inv_start_from_history=neighbortrack_args['inv_start_from_history'],history_dict=neighbortrack_args['history_dict'],max_consecutive=neighbortrack_args['max_consecutive'],pred_margin_ratio=neighbortrack_args['pred_margin_ratio'],neighbor_search_ratio=neighbortrack_args['neighbor_search_ratio'],neighbor_score_only=neighbortrack_args['neighbor_score_only'],neighbor_static_th=neighbortrack_args['neighbor_static_th'],neighbor_th=neighbortrack_args['neighbor_th'],group_inv=neighbortrack_args['group_inv'])

        new_pos = self.siam_state['target_pos'][::-1]
