...
class CenterPredictor(nn.Module, ):
    ...
    def cal_bbox_neighbor(self, score_map_ctr, size_map, offset_map, return_score=False,th=0.7):
        
        
        
        max_score, idx = torch.max(score_map_ctr.clone().flatten(1), dim=1, keepdim=True)

        c_score_map = score_map_ctr.clone().flatten(1)
        #c_score_map[0][idx] = 0
        
        mask = c_score_map.ge(th*max_score)
        #print(mask)
        value = torch.masked_select(c_score_map,mask)
        indexes = torch.nonzero(mask, as_tuple=True)[1]
        #print(value)
        #print(indexes)
        #n_bbox = []
        n_bbox=torch.tensor([]).cuda()

        n_score = value
        for index in indexes:
            index = index.clone().unsqueeze(0).unsqueeze(0)
            
            idx_y = index // self.feat_sz
            idx_x = index % self.feat_sz
            
            index = index.unsqueeze(1).expand(index.shape[0], 2, 1)
            size = size_map.flatten(2).gather(dim=2, index=index)
            offset = offset_map.flatten(2).gather(dim=2, index=index).squeeze(-1)
            ...
            bbox = torch.cat([(idx_x.to(torch.float) + offset[:, :1]) / self.feat_sz,
                              ...
                              size.squeeze(-1)], dim=1)
            #n_bbox.append(bbox)
            n_bbox = torch.cat((n_bbox,bbox),0)
        #print(idx_y)
        #print(idx_x)

        
        '''
        c_score_vec = score_map_ctr.clone()
        value , index = torch.max((c_prob_vec), dim=1)
        n_bbox = []
        n_score = []
        for sub_score_vec in c_score_vec:
            for index_morethenth in range(index_whiletime):
                sub_score_vec[index]=-1e+31
                value , index = torch.max((sub_score_vec), dim=0)
                minus_prob_vec = nn.functional.softmax(c_score_vec, dim=1)
                max_score, idx = torch.max(minus_prob_vec.flatten(1), dim=1, keepdim=True)
                idx_y = idx // self.feat_sz
                idx_x = idx % self.feat_sz
...
def build_box_head(cfg, hidden_dim):
    ...
    if cfg.MODEL.HEAD.TYPE == "MLP":
        ...
        if cfg.MODEL.HEAD.TYPE == "CORNER":
            ...
            raise ValueError()
        return corner_head
    elif cfg.MODEL.HEAD.TYPE == "CENTER" or cfg.MODEL.HEAD.TYPE == "CENTER_NEIGHBOR":
